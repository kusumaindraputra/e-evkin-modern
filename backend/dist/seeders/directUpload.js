...
async function uploadExcel() {
    ...
    for (const [key, group] of grouped) {
        try {
            ...
            let puskesmas = await models_1.User.findOne({
                where: {
            ...
            if (!puskesmas) {
                ...
                puskesmas = await models_1.User.findOne({
                    where: { nama: { [sequelize_1.Op.iLike]: searchName }, role: 'puskesmas' },
                });
            }
            if (!puskesmas) {
...
async function uploadPdf() {
    ...
    const parsed = await (0, angkasParserService_1.parseAngkasPdf)(buffer);
    ...
    for (const puskesmasData of parsed.puskesmasList) {
        const userId = (0, angkasParserService_1.findPuskesmasUser)(puskesmasData.namaPuskesmas, puskesmasUsers.map(u => ({
            id: u.id,
            nama: u.nama,
            username: u.username,
    ...
    const existingRecords = await models_1.AnggaranKas.findAll({
        where: { tahun },
        attributes: ['id', 'user_id', 'kode_rekening', 'id_sumber_anggaran', 'bulan'],
        raw: true,
    });
    // Create a Set for quick lookup
    const existingSet = new Set(existingRecords.map(r => `${r.user_id}-${r.kode_rekening}-${r.id_sumber_anggaran}-${r.bulan}`));
    const existingMap = new Map(existingRecords.map(r => [
    ...
    if (toInsert.length > 0) {
        ...
        for (let i = 0; i < toInsert.length; i += BATCH_SIZE) {
            ...
            await models_1.AnggaranKas.bulkCreate(batch, { logging: false });
    ...
    if (toUpdate.length > 0) {
        ...
        for (let i = 0; i < toUpdate.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map(({ id, nilai }) => models_1.AnggaranKas.update({ nilai }, { where: { id }, logging: false })));
