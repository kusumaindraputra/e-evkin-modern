...
const AdminTargetEditPage: React.FC = () => {
  ...
  const targetColumns = [
    ...
    { title: 'Tahun', dataIndex: 'tahun', key: 'tahun', width: 70 },
    {
      ...
      render: (value: number) => <span style={{ color: value === 0 ? 'var(--c-warn)' : undefined }}>{formatNumber(value)}</span>,
    },
    {
  ...
  const angkasColumns = [
    ...
    { title: 'Target Anggaran (Rp)', dataIndex: 'target_rp', key: 'target_rp', width: 130, align: 'right' as const, render: (value: number) => formatNumber(value) },
    {
      ...
      render: (value: number, record: AngkasData) => (
        <span style={{ color: value === 0 ? 'var(--c-warn)' : undefined }}>
          {formatNumber(value)}
  ...
  return (
    <div style={{ padding: '24px' }}>
          ...
          },
          {
            ...
            children: (
              <>
                ...
                <p style={{ color: 'var(--c-txt-3)', fontSize: '12px', marginBottom: 16 }}>
      ...
      <Modal title="Edit Target Kinerja" open={targetEditModalVisible}
        ...
        {selectedTarget && (
          <div style={{ marginBottom: 16, padding: 12, background: 'var(--c-bg)', borderRadius: 6 }}>
        ...
        <Timeline mode="left" items={targetHistoryData.map((record) => ({
          ...
          children: (
            <div>
              ...
              <div>Satuan: <strong>{record.satuan?.satuannya || <i style={{ color: 'var(--c-txt-3)' }}>Belum dipilih</i>}</strong></div>
              <div style={{ fontSize: '12px', color: 'var(--c-txt-3)' }}>Target Anggaran (Rp): {formatNumber(record.target_rp)}</div>
      ...
      <Modal title="Edit Angkas" open={angkasEditModalVisible}
        ...
        {selectedAngkas && (
          <div style={{
            ...
            border: `1px solid ${isValidAngkasTotal() ? 'var(--c-success)' : 'var(--c-err)'}`,
            borderRadius: 6
          }}>
            <Row gutter={16}>
              <Col span={8}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: 'var(--c-txt-3)', fontSize: 12 }}>Total Angkas</div>
                  <div style={{ fontSize: 16, fontWeight: 'bold' }}>{formatNumber(angkasFormTotal)}</div>
                </div>
              </Col>
              <Col span={8}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: 'var(--c-txt-3)', fontSize: 12 }}>Target Anggaran</div>
                  <div style={{ fontSize: 16, fontWeight: 'bold' }}>{formatNumber(selectedAngkas.target_rp)}</div>
                </div>
              </Col>
              <Col span={8}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: 'var(--c-txt-3)', fontSize: 12 }}>Selisih</div>
                  <div style={{
                    fontSize: 16,
                    fontWeight: 'bold',
                    color: getAngkasSelisih() === 0 ? 'var(--c-success)' : 'var(--c-err)'
                  }}>
            ...
            {!isValidAngkasTotal() && (
              <div style={{ marginTop: 12, padding: '8px 12px', background: 'var(--bg-warning-subtle)', border: `1px solid var(--c-warn)`, borderRadius: 4 }}>
                <span style={{ color: 'var(--c-warn)' }}>⚠️ Total angkas harus sama dengan target anggaran untuk dapat menyimpan</span>
              </div>
            )}
          </div>
        )}
      ...
      <Modal title="History Angkas" open={angkasHistoryModalVisible} onCancel={() => { setAngkasHistoryModalVisible(false); setSelectedAngkas(null); }}
        ...
        {angkasHistoryData.length === 0 ? (
          <p style={{ textAlign: 'center', color: 'var(--c-txt-3)' }}>Belum ada history perubahan</p>
        ) : (
          <Timeline mode="left" items={angkasHistoryData.map((record) => ({
            ...
            children: (
              <div>
                ...
                <div style={{ fontSize: '12px', color: 'var(--c-txt-3)' }}>Dibuat oleh: {record.creator?.nama || 'N/A'}</div>
                {record.uraian && <div style={{ marginTop: 6, padding: '6px 10px', background: 'var(--c-bg)', borderRadius: 4, fontSize: '12px' }}><strong>Catatan:</strong> {record.uraian}</div>}
              </div>
            ),
          }))} />
        )}
