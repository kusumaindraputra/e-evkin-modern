...
export const ChatWidget: React.FC<ChatWidgetProps> = ({ compact = false }) => {
  ...
  if (compact) {
    return (
      <Card
        ...
        title={
          <Space size="small">
            <RobotOutlined style={{ color: 'var(--c-prim)', fontSize: 14 }} />
              ...
              {messages.map((msg) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '6px 8px',
                    borderRadius: 4,
                    backgroundColor: msg.type === 'user' ? 'var(--bg-info-subtle)' : 'var(--c-bg)',
                    borderLeft: `2px solid ${msg.type === 'user' ? 'var(--c-prim)' : 'var(--c-success)'}`,
                    fontSize: 12,
                  }}
                >
                  {msg.type === 'assistant' ? (
  ...
  return (
    <Card
      ...
      title={
        <Space>
          <RobotOutlined style={{ color: 'var(--c-prim)' }} />
      ...
      >
        {messages.length === 0 ? (
          <div style={{ padding: '20px 0' }}>
            <p style={{ color: 'var(--c-txt-2)', fontSize: 13, marginBottom: 16 }}>
          ...
          <>
            {messages.map((msg) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 6,
                  backgroundColor: msg.type === 'user' ? 'var(--bg-info-subtle)' : 'var(--c-bg)',
                  borderLeft: `3px solid ${msg.type === 'user' ? 'var(--c-prim)' : 'var(--c-success)'}`,
                }}
              >
                <p style={{ margin: 0, marginBottom: 4, fontWeight: 500, fontSize: 12, color: 'var(--c-txt-2)' }}>
            ...
            {loading && (
              <div style={{ textAlign: 'center', padding: 12 }}>
                <Spin size="default" />
                <p style={{ marginTop: 8, color: 'var(--c-txt-2)', fontSize: 12 }}>Menganalisis data...</p>
              </div>
            )}
            <div ref={messagesEndRef} />
          </>
