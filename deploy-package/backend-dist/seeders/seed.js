...
async function seed() {
    try {
        await database_1.sequelize.sync({ force: true }); // This will drop and recreate tables
        ...
        for (const satuan of satuanData) {
            await Satuan_1.default.create(satuan);
        }
        ...
        for (const sumber of sumberAnggaranData) {
            await SumberAnggaran_1.default.create(sumber);
        }
        ...
        for (const kegiatan of kegiatanData) {
            await Kegiatan_1.default.create(kegiatan);
        }
        ...
        for (const subKegiatan of subKegiatanData) {
            await SubKegiatan_1.default.create(subKegiatan);
        }
        ...
        const subKegiatanMap = await SubKegiatan_1.default.findAll({
            ...
            for (const month of months) {
                ...
                for (let i = 0; i < numReports; i++) {
                    ...
                    laporanBulkData.push({
                        ...
                        bulan: month,
                        tahun: 2025,
                        status: 'terkirim', // Set status ke terkirim agar muncul di dashboard
                    });
                }
            }
        }
        // Insert all laporan in single batch operation
        await Laporan_1.default.bulkCreate(laporanBulkData);
