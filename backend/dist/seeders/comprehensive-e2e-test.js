...
let failedTests = 0;
// Utility functions
function log(step, success, message, data, duration) {
...
// Fallback wrapper
async function withFallback(operation, fallback, errorMessage) {
    try {
...
async function loginAdmin() {
    ...
    try {
        const { result, duration } = await measureTime(async () => {
            const response = await axios_1.default.post(`${CONFIG.API_BASE}/auth/login`, CONFIG.ADMIN_CREDENTIALS);
...
async function cleanData() {
    ...
    try {
        ...
        const { result: deletedLaporan, duration: d1 } = await measureTime(async () => {
            return await models_1.Laporan.destroy({ where: { tahun: CONFIG.TAHUN } });
        });
        ...
        const { result: deletedTargets, duration: d2 } = await measureTime(async () => {
            return await models_1.SubKegiatanTarget.destroy({ where: { tahun: CONFIG.TAHUN } });
        });
        ...
        const { result: deletedAngkas, duration: d3 } = await measureTime(async () => {
            return await models_1.AnggaranKas.destroy({ where: { tahun: CONFIG.TAHUN } });
        });
...
async function verifyReferenceData() {
    ...
    try {
        ...
        const puskesmasList = await models_1.User.findAll({
...
// =============================================================================
async function uploadExcelTarget() {
    console.log('\n' + '='.repeat(60));
    ...
    try {
        ...
        const { result, duration } = await measureTime(async () => {
            const response = await axios_1.default.post(`${CONFIG.API_BASE}/target/upload`, formData, {
                headers: {
                    ...formData.getHeaders(),
        ...
        if (result.success) {
            ...
            log('Excel Upload', true, `Inserted: ${inserted}, Updated: ${updated}, Skipped: ${skipped}`, null, duration);
...
async function createTargetsManually() {
    ...
    try {
        ...
        const satuan = await models_1.Satuan.findOne();
        let created = 0;
        const { duration } = await measureTime(async () => {
            for (const puskesmas of puskesmasList) {
                for (const subKegiatan of subKegiatanList) {
                    const sumberList = subKegiatan.sumberAnggaranList || [];
...
// =============================================================================
async function uploadPdfAngkas() {
    console.log('\n' + '='.repeat(60));
    ...
    try {
        ...
        const { result, duration } = await measureTime(async () => {
            const response = await axios_1.default.post(`${CONFIG.API_BASE}/angkas/upload`, formData, {
                headers: {
                    ...formData.getHeaders(),
                    Authorization: `Bearer ${authToken}`,
                },
        ...
        if (result.success) {
            const uploadData = result.data || result.result || {};
            log('PDF Upload', true, `Inserted: ${uploadData.inserted || 0}, Updated: ${uploadData.updated || 0}`, null, duration);
            return { success: true, data: result };
        }
        else {
...
async function checkUploadHistory() {
    ...
    try {
        ...
        const targetHistory = await withFallback(async () => {
            const response = await axios_1.default.get(`${CONFIG.API_BASE}/target/history`, {
                headers: { Authorization: `Bearer ${authToken}` },
                params: { tahun: CONFIG.TAHUN }
            });
            return response.data;
        }, { history: [] }, 'Target history API not available');
        log('Target History', true, `Found ${targetHistory.history?.length || 0} upload history records`);
        // Check angkas history via API
        const angkasHistory = await withFallback(async () => {
            const response = await axios_1.default.get(`${CONFIG.API_BASE}/angkas/history`, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
            return response.data;
        }, { history: [] }, 'Angkas history API not available');
        log('Angkas History', true, `Found ${angkasHistory.history?.length || 0} upload history records`);
        // Verify data in database
        const targetCount = await models_1.SubKegiatanTarget.count({ where: { tahun: CONFIG.TAHUN } });
        log('Target Count', true, `${targetCount} targets in database for ${CONFIG.TAHUN}`);
        const angkasCount = await models_1.AnggaranKas.count({ where: { tahun: CONFIG.TAHUN } });
...
// =============================================================================
async function createLaporanAllPuskesmas() {
    console.log('\n' + '='.repeat(60));
    ...
    try {
        ...
        const targets = await models_1.SubKegiatanTarget.findAll({
            ...
            include: [
                { model: models_1.SubKegiatan, as: 'subKegiatan', include: [{ model: models_1.Kegiatan, as: 'kegiatanParent' }] },
                { model: models_1.SumberAnggaran, as: 'sumberAnggaran' },
                { model: models_1.Satuan, as: 'satuan' },
            ]
        });
        if (targets.length === 0) {
            ...
            for (const bulan of CONFIG.BULAN_LIST) {
                ...
                for (const [userId, userTargets] of targetsByUser) {
                    for (const target of userTargets) {
                        const subKegiatan = target.subKegiatan;
                        const sumberAnggaran = target.sumberAnggaran;
        ...
        const labkesda = await models_1.User.findOne({ where: { username: 'labkesda' } });
        if (labkesda) {
            const labkesdaLaporan = await models_1.Laporan.count({
                where: { user_id: labkesda.id, tahun: CONFIG.TAHUN }
            });
            log('Labkesda Laporan', labkesdaLaporan > 0, labkesdaLaporan > 0 ? `Labkesda has ${labkesdaLaporan} laporan` : 'Labkesda has no laporan');
        }
        return true;
    }
    catch (error) {
...
async function verifyDashboardData() {
    ...
    try {
        // Get dashboard stats
        const stats = await models_1.Laporan.findAll({
            where: { tahun: CONFIG.TAHUN, status: 'terkirim' },
            attributes: [
                'bulan',
                [models_1.sequelize.fn('SUM', models_1.sequelize.col('target_rp')), 'total_target'],
                [models_1.sequelize.fn('SUM', models_1.sequelize.col('realisasi_rp')), 'total_realisasi'],
        ...
        for (const stat of stats) {
            ...
            const target = Number(stat.dataValues.total_target);
            const realisasi = Number(stat.dataValues.total_realisasi);
        ...
        // Overall totals
        const totals = await models_1.Laporan.findOne({
            where: { tahun: CONFIG.TAHUN, status: 'terkirim' },
            attributes: [
                [models_1.sequelize.fn('SUM', models_1.sequelize.col('target_rp')), 'total_target'],
                [models_1.sequelize.fn('SUM', models_1.sequelize.col('realisasi_rp')), 'total_realisasi'],
                [models_1.sequelize.fn('COUNT', models_1.sequelize.col('id')), 'total_laporan'],
            ],
        });
        const totalTarget = Number(totals?.dataValues.total_target || 0);
        const totalRealisasi = Number(totals?.dataValues.total_realisasi || 0);
        const totalLaporan = Number(totals?.dataValues.total_laporan || 0);
        ...
        const puskesmasReporting = await models_1.Laporan.findAll({
            where: { tahun: CONFIG.TAHUN, status: 'terkirim' },
            attributes: [
                [models_1.sequelize.fn('DISTINCT', models_1.sequelize.col('user_id')), 'user_id'],
            ],
        });
...
async function checkOptimizations() {
    ...
    try {
        ...
        const { duration: queryDuration } = await measureTime(async () => {
            await models_1.Laporan.findAll({
                where: { tahun: CONFIG.TAHUN },
                include: [
                    { model: models_1.SubKegiatan, as: 'subKegiatan' },
                    { model: models_1.SumberAnggaran, as: 'sumberAnggaran' },
                ],
                limit: 100,
            });
        ...
        log('Query Performance', queryOptimal, `Laporan query: ${queryDuration}ms ${queryOptimal ? '(optimal)' : '(needs optimization)'}`);
        ...
        log('Aggregate Performance', aggOptimal, `Aggregate query: ${aggDuration}ms ${aggOptimal ? '(optimal)' : '(needs optimization)'}`);
        // Check database indexes
        const indexCheck = await withFallback(async () => {
        ...
        log('Database Indexes', true, `Found ${indexCheck.length} indexes on main tables`);
        // Memory usage check
...
// Run the tests
runTests();
//# sourceMappingURL=comprehensive-e2e-test.js.map