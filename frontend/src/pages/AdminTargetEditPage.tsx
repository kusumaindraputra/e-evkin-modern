...
const AdminTargetEditPage: React.FC = () => {
  ...
  const targetColumns = [
    ...
    { title: 'Tahun', dataIndex: 'tahun', key: 'tahun', width: 70 },
    {
      ...
      render: (value: number) => <span style={{ color: value === 0 ? '#faad14' : undefined }}>{formatNumber(value)}</span>,
    },
    {
    ...
    <div style={{ padding: '24px' }}>
        ...
        <Timeline mode="left" items={targetHistoryData.map((record) => ({
          ...
          children: (
            <div>
              ...
              <div>Satuan: <strong>{record.satuan?.satuannya || <i style={{ color: '#999' }}>Belum dipilih</i>}</strong></div>
              <div style={{ fontSize: '12px', color: '#888' }}>Target Anggaran (Rp): {formatNumber(record.target_rp)}</div>
      ...
      <Modal title="Edit Angkas" open={angkasEditModalVisible}
        ...
        {selectedAngkas && (
          <div style={{ 
            ...
            {!isValidAngkasTotal() && (
              <div style={{ marginTop: 12, padding: '8px 12px', background: '#fffbe6', border: '1px solid #ffe58f', borderRadius: 4 }}>
                <span style={{ color: '#d48806' }}>⚠️ Total angkas harus sama dengan target anggaran untuk dapat menyimpan</span>
              </div>
            )}
          </div>
        )}
      ...
      <Modal title="History Angkas" open={angkasHistoryModalVisible} onCancel={() => { setAngkasHistoryModalVisible(false); setSelectedAngkas(null); }}
        ...
        {angkasHistoryData.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#888' }}>Belum ada history perubahan</p>
        ) : (
          <Timeline mode="left" items={angkasHistoryData.map((record) => ({
            ...
            children: (
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#888' }}>Dibuat oleh: {record.creator?.nama || 'N/A'}</div>
                {record.uraian && <div style={{ marginTop: 6, padding: '6px 10px', background: '#f5f5f5', borderRadius: 4, fontSize: '12px' }}><strong>Catatan:</strong> {record.uraian}</div>}
              </div>
            ),
          }))} />
        )}
