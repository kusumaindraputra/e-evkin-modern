...
exports.measureFrontendBundleSize = measureFrontendBundleSize;
exports.measureBackendEndpoints = measureBackendEndpoints;
exports.measureDatabaseMetrics = measureDatabaseMetrics;
...
async function measureFrontendBundleSize() {
...
async function measureBackendEndpoints() {
    ...
    try {
        ...
        const refTime = (await measureTime(async () => {
            return axios_1.default.get(`${API_BASE_URL}/reference/sumber-anggaran`, { headers });
        })).duration;
        console.log(`  ✓ /reference/sumber-anggaran: ${refTime.toFixed(2)}ms`);
        // 2. Laporan List Query
        const tahun = new Date().getFullYear();
        const laporanTime = (await measureTime(async () => {
            return axios_1.default.get(`${API_BASE_URL}/laporan?tahun=${tahun}&bulan=1`, { headers });
        })).duration;
        console.log(`  ✓ /laporan?tahun=${tahun}&bulan=1: ${laporanTime.toFixed(2)}ms`);
        // 3. Dashboard Query
        const dashboardTime = (await measureTime(async () => {
            return axios_1.default.get(`${API_BASE_URL}/report/dashboard?tahun=${tahun}`, { headers });
        })).duration;
        ...
        const bulkTime = (await measureTime(async () => {
            return axios_1.default.post(`${API_BASE_URL}/laporan/bulk-upsert`, { laporan: bulkData }, { headers });
        })).duration;
...
async function measureConnectionPool() {
