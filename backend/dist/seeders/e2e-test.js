...
const results = [];
function log(step, success, message, data) {
...
async function cleanData() {
    ...
    try {
        ...
        const deletedLaporan = await models_1.Laporan.destroy({
            where: { tahun: TAHUN }
        });
        log('Clean Laporan', true, `Deleted ${deletedLaporan} laporan records for tahun ${TAHUN}`);
        // Delete targets for test year
        const deletedTargets = await models_1.SubKegiatanTarget.destroy({
            where: { tahun: TAHUN }
        });
...
async function inputTargets(testData) {
    ...
    try {
        for (const puskesmas of puskesmasList) {
            for (const subKegiatan of subKegiatanList) {
                const sumberAnggaranList = subKegiatan.sumberAnggaranList || [];
                ...
                for (const sumberAnggaran of sumberAnggaranList) {
                    ...
                    const target = await models_1.SubKegiatanTarget.create({
        ...
        // Verify targets
        const targetCount = await models_1.SubKegiatanTarget.count({ where: { tahun: TAHUN } });
        log('Verify Targets', targetCount === targetsCreated.length, `Target count in DB: ${targetCount}, Expected: ${targetsCreated.length}`);
...
async function inputLaporan(testData, targetsCreated) {
    console.log('\n========== STEP 4: INPUT LAPORAN ==========\n');
    const { puskesmasList, subKegiatanList, satuan } = testData;
    const laporanCreated = [];
    ...
    try {
        for (const bulan of BULAN_LIST) {
            ...
            for (const puskesmas of puskesmasList) {
                for (const subKegiatan of subKegiatanList) {
                    ...
                    for (const sumberAnggaran of sumberAnggaranList) {
                        ...
                        const laporan = await models_1.Laporan.create({
                            user_id: puskesmas.id,
            ...
            log(`Input Laporan ${bulan}`, true, `Created laporan for ${bulan} with ${percentage * 100}% realisasi`);
        }
        // Verify laporan count
        const laporanCount = await models_1.Laporan.count({ where: { tahun: TAHUN } });
        // @ts-ignore - implicit types ok for seeder
        const expectedCount = BULAN_LIST.length * puskesmasList.length *
            subKegiatanList.reduce((sum, sk) => sum + (sk.sumberAnggaranList?.length || 0), 0);
        log('Verify Laporan Count', laporanCount === expectedCount, `Laporan count in DB: ${laporanCount}, Expected: ${expectedCount}`);
...
async function verifyDashboardData(testData) {
    ...
    try {
        ...
        const budgetByMonth = await models_1.Laporan.findAll({
            attributes: [
                'bulan',
        ...
        log('Budget by Month', budgetByMonth.length === BULAN_LIST.length, `Found ${budgetByMonth.length} months of data`);
        // 2. Verify total statistics
        console.log('\n--- Total Statistics ---');
        const totalStats = await models_1.Laporan.findOne({
            attributes: [
                [models_1.sequelize.fn('SUM', models_1.sequelize.col('target_rp')), 'total_target'],
                [models_1.sequelize.fn('SUM', models_1.sequelize.col('realisasi_rp')), 'total_realisasi'],
        ...
        // @ts-ignore - implicit types ok for seeder
        const numSumberAnggaran = subKegiatanList.reduce((sum, sk) => sum + (sk.sumberAnggaranList?.length || 0), 0);
        ...
        log('Verify Total Target', targetMatch, `Actual: ${total_target}, Expected: ${expectedTotalTarget}`);
        log('Verify Total Realisasi', realisasiMatch, `Actual: ${total_realisasi}, Expected: ${expectedTotalRealisasi}`);
        ...
        log('Puskesmas Reporting', puskesmasReporting === puskesmasList.length, `Reporting: ${puskesmasReporting}, Expected: ${puskesmasList.length}`);
        ...
        log('All Laporan Submitted', allSubmitted, `All laporan should be 'terkirim'`);
        return true;
    }
    catch (error) {
