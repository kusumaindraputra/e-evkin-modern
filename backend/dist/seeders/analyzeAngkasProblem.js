...
async function analyze() {
    ...
    const user = await User_1.default.findOne({
        where: { nama_puskesmas: { [sequelize_1.Op.iLike]: '%babakan madang%' } }
    });
    if (!user) {
    ...
    const subKegiatan = await SubKegiatan_1.default.findOne({
    ...
    const subKegSumberDana = await database_1.default.query(`
  ...
  `, {
    ...
    subKegSumberDana.forEach((s) => {
        console.log(`   - ${s.sumber} (id: ${s.id_sumber_anggaran})`);
    });
    // 4. Get targets for this sub kegiatan
    const targets = await SubKegiatanTarget_1.default.findAll({
        where: {
            user_id: user.id,
            id_sub_kegiatan: subKegiatan.id_sub_kegiatan,
            tahun: 2025
        },
        include: [{ model: SumberAnggaran_1.default, as: 'sumberAnggaran' }]
    });
    ...
    const angkas = await database_1.default.query(`
    SELECT ak.*, sa.sumber 
...
`);
    ...
    const multiSumberSubKegiatan = await database_1.default.query(`
    SELECT 
      sk.id_sub_kegiatan, 
      sk.kode_sub, 
      sk.kegiatan,
  ...
  `, { type: 'SELECT' });
    ...
    const duplicateAngkas = await database_1.default.query(`
    SELECT 
      ak.user_id,
      ak.id_sub_kegiatan,
      ak.tahun,
...
//# sourceMappingURL=analyzeAngkasProblem.js.map