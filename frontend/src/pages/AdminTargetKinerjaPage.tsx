...
const AdminTargetKinerjaPage: React.FC = () => {
  ...
  const loadReferenceData = async () => {
    try {
      ...
      // API already returns {value, label} format
    ...
    <div style={{ padding: '24px' }}>
      <Card title="Target Kinerja (K)" style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: '#888', fontSize: '12px', marginBottom: 16 }}>
          * Target dengan nilai 0 dan satuan "Silahkan Pilih" perlu diupdate
        </p>
      ...
      >
        ...
        <Form form={form} layout="vertical">
          ...
          >
            <Input.TextArea
              rows={3}
              placeholder="Contoh: Update target kinerja sesuai RAK / Penyesuaian satuan"
            />
          </Form.Item>
        </Form>
      </Modal>
          ...
          items={historyData.map((record) => ({
            ...
            children: (
              <div>
                ...
                <div>
                  Satuan: <strong>{getSatuanName(record.id_satuan) || <i style={{ color: '#999' }}>Belum dipilih</i>}</strong>
                </div>
...
export default AdminTargetKinerjaPage;
